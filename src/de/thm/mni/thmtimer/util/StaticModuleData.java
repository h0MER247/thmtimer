...
public class StaticModuleData {
	...
	private static void createCourseList() {
		...
		if(m_courses == null) {
			...
			m_courses.add(new Course(0l,									// KursID (so wie sie in der Serverdatenbank stehen würde)
									 0l,									// ModulID (so wie sie in der Serverdatenbank stehen würde) -> "Verbindet" den Kurs mit dem dazugehörigen Modul
	...
	private static void createTimeCategorys() {
		...
		if(m_timeCategorys == null) {
			...
			m_timeCategorys.add(new TimeCategory(0l,			// KategorieID (so wie sie in der Serverdatenbank stehen würde)
												 "Vorlesung"));	// Bezeichnung für diese Kategorie
	...
	private static void createStudentData() {
		...
		if(m_studentData == null) {
			...
			// Timetrackingdaten für:
			// OOP
			m_studentData.addTimeTracking(0l,												// KursID (so wie sie in der Datenbank stehen würde)
										  new TimeTracking(0l,								// TimeTrackingID (so wie sie in der Datenbank stehen würde) -> Um Zeiten evtl. zu bearbeiten, löschen, etc...
												           0l,								// CategoryID (so wie sie in der Datenbank stehen würde) -> Ordnet diesem Tracking eine Kategorie zu (Hausaufgaben, Praktika, Vorlesung, etc...)
												           "War heute in der Vorlesung",	// Eigener Text für dieses Tracking
			...
			m_studentData.addTimeTracking(2l, new TimeTracking(6l, 1l, "War im Praktikum... Es läuft...", new TimeData("1:36")));
	...
	private static void createTeacherData() {
		...
		if(m_teacherData == null) {
			...
			// Zeitstatistikdaten für
			// Compilerbau
			m_teacherData.addTimeStatistic(2l,												// KursID (so wie sie in der Datenbank stehen würde)
										   new TimeStatisticData(0l,						// KategorieID (so wie sie in der Datenbank stehen würde)
